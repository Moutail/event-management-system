  ...
  const confirmAction = async () => {
    try {
      ...
      if (actionType === 'bulk_approve' || actionType === 'bulk_reject') {
        ...
        response = await api.post('/admin/bulk_process_refunds/', payload);
      } else {
        // Action individuelle
        const payload = {
        ...
        response = await api.post('/admin/process_refund/', payload);
      }
              ...
              <TableRow key={refund.id}>
                ...
                <TableCell>
                  <Box display="flex" alignItems="center" gap={1}>
                    <Avatar sx={{ width: 32, height: 32 }}>
                      {refund.registration?.user?.username?.[0]?.toUpperCase() || '?'}
                    </Avatar>
                    <Box>
                ...
                <TableCell>
                  <Box>
                    ...
                    <Typography variant="body2" color="textSecondary" fontSize="0.75rem">
                      {refund.registration?.event?.start_date ? 
                        new Date(refund.registration.event.start_date).toLocaleDateString('fr-FR') : 
                        'Date non spécifiée'
                      }
                    </Typography>
                  </Box>
