#!/usr/bin/env python3
"""
Test COMPLET de la logique corrigée
"""
...
def test_complete_fix():
    """Test complet de la logique corrigée"""
    print("🧪 Test COMPLET de la logique corrigée")
    print("=" * 60)
    
    # Créer des utilisateurs de test
    user1, created = User.objects.get_or_create(
        username='test_user1_complete',
        defaults={
            'email': 'test1@complete.com',
            'first_name': 'Test1',
            'last_name': 'Complete'
        }
    )
    ...
    user2, created = User.objects.get_or_create(
        username='test_user2_complete',
        defaults={
            'email': 'test2@complete.com',
            'first_name': 'Test2',
            'last_name': 'Complete'
        }
    )
    ...
    user3, created = User.objects.get_or_create(
        username='test_user3_complete',
        defaults={
            'email': 'test3@complete.com',
            'first_name': 'Test3',
            'last_name': 'Complete'
        }
    )
    ...
    event, created = Event.objects.get_or_create(
        title='Test Complet Corrigé',
        defaults={
            'description': 'Test complet de la logique corrigée',
            'short_description': 'Test complet corrigé',
    ...
    reg4 = EventRegistration.objects.create(
        event=event,
        user=User.objects.create(username='test_user4_complete'),
        ticket_type=ticket_vip,
        session_type=session_soir,
        status='confirmed',
        payment_status='paid',
    ...
    # Test 7: Vérifier que l'API des sessions fonctionne
    print(f"\n🔍 Test 7: Test de l'API des sessions")
    try:
        from events.serializers import SessionTypeSerializer
        serializer = SessionTypeSerializer(session_matin)
        data = serializer.data
        print(f"   ✅ SessionTypeSerializer fonctionne:")
        print(f"     * ID: {data.get('id')}")
        print(f"     * Nom: {data.get('name')}")
        print(f"     * Places disponibles: {data.get('available_spots')}")
        print(f"     * Participants confirmés: {data.get('confirmed_participants')}")
    ...
    user2.delete()
    user3.delete()
    User.objects.filter(username='test_user4_complete').delete()
    ...
    print(f"   7. L'API des sessions fonctionne correctement")
