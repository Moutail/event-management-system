#!/usr/bin/env python3
"""
Test de la nouvelle logique de comptage séparé des billets et sessions
"""
...
def test_new_counting_logic():
    """Test de la nouvelle logique de comptage séparé"""
    print("🧪 Test de la nouvelle logique de comptage séparé")
    print("=" * 60)
    
    # Créer des utilisateurs de test
    user1, created = User.objects.get_or_create(
        username='test_user1_counting',
        defaults={
            'email': 'test1@counting.com',
            'first_name': 'Test1',
            'last_name': 'Counting'
        }
    )
    ...
    user2, created = User.objects.get_or_create(
        username='test_user2_counting',
        defaults={
            'email': 'test2@counting.com',
            'first_name': 'Test2',
            'last_name': 'Counting'
        }
    )
    ...
    user3, created = User.objects.get_or_create(
        username='test_user3_counting',
        defaults={
            'email': 'test3@counting.com',
            'first_name': 'Test3',
            'last_name': 'Counting'
        }
    )
    ...
    if created:
        ...
        print(f"✅ Utilisateurs de test existants")
    
    # Créer un événement de test
    event, created = Event.objects.get_or_create(
        title='Test Comptage Séparé',
        defaults={
            'description': 'Test de la nouvelle logique de comptage',
            'short_description': 'Test comptage',
            'start_date': timezone.now() + timezone.timedelta(days=30),
            'end_date': timezone.now() + timezone.timedelta(days=30, hours=2),
            'location': 'Test Location',
            'address': 'Test Address',
            'place_type': 'limited',
            'max_capacity': 10,  # 10 places totales
            'price': Decimal('20.00'),
            'is_free': False,
            'event_type': 'physical',
            'status': 'published',
    ...
    if created:
        ...
        print(f"✅ Événement de test existant: {event.title}")
    ...
    print(f"   - VIP: {ticket_vip.quantity} places")
    print(f"   - Standard: {ticket_standard.quantity} places")
    ...
    print(f"   1. Billets par défaut: Compteur séparé basé sur max_capacity")
    ...
    print(f"   5. Les compteurs sont INDÉPENDANTS et se basent sur les inscriptions confirmées")
