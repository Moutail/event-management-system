#!/usr/bin/env python
"""
Debug spécifique de l'erreur created_at
"""
import os
import django
import traceback
...
from events.models import UserProfile, Event, EventRegistration, RefundRequest
from events.views import super_admin_global_stats

def debug_created_at():
    """Debug spécifique de l'erreur created_at"""
    print("🔍 Debug spécifique de l'erreur created_at...")
    ...
    try:
        ...
        # 2. Test des modèles et relations
        print(f"\n🧪 Test des modèles et relations...")
        
        # Test 1: Vérifier EventRegistration
        try:
            reg = EventRegistration.objects.first()
            if reg:
                print(f"   ✅ EventRegistration trouvé: {reg.id}")
                print(f"   📋 Champs disponibles: {[f.name for f in reg._meta.fields]}")
            else:
                print(f"   ❌ Aucun EventRegistration trouvé")
        except Exception as e:
            print(f"   ❌ Erreur EventRegistration: {e}")
            traceback.print_exc()
            return
        
        # Test 2: Vérifier RefundRequest
        try:
            refund = RefundRequest.objects.first()
            if refund:
                print(f"   ✅ RefundRequest trouvé: {refund.id}")
                print(f"   📋 Champs disponibles: {[f.name for f in refund._meta.fields]}")
            else:
                print(f"   ❌ Aucun RefundRequest trouvé")
        except Exception as e:
            print(f"   ❌ Erreur RefundRequest: {e}")
            traceback.print_exc()
            return
        
        # Test 3: Vérifier Event
        try:
            event = Event.objects.first()
            if event:
                print(f"   ✅ Event trouvé: {event.id}")
                print(f"   📋 Champs disponibles: {[f.name for f in event._meta.fields]}")
            else:
                print(f"   ❌ Aucun Event trouvé")
        except Exception as e:
            print(f"   ❌ Erreur Event: {e}")
            traceback.print_exc()
            return
        
        # Test 4: Test de la vue avec gestion d'erreur détaillée
        print(f"\n🚀 Test de la vue avec gestion d'erreur détaillée...")
        try:
